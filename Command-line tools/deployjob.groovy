...
while(groupId == null || groupId.trim().equals("")) {
	println "Enter bundle groupId: " ;
	groupId = reader.readLine()
}
while(artifactId == null || artifactId.trim().equals("")) {
	println "Enter bundle artifactId: " ;
	artifactId = reader.readLine()
}
while(version == null || version.trim().equals("")) {
	println "Enter bundle version: " ;
	version = reader.readLine()
}
println "Enter wiperdog host (default is : \"localhost\"): " ;
def host = reader.readLine()
if(host == null || host.trim().equals("")) {
	host = "localhost"
}
println "Enter Rest post (default is : 8089): " ;
	...
	try{
		...
		if(objData.status.equals("success")) {
			...
			while(true) {
				...
				println "Enter 'all' for install all files:  Left empty for exit ."
				def listInstallFile = []
